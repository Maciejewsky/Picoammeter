...
class ResistanceMeasurementBlock(QWidget):
    ...
    def __init__(self, instrument, user_manager=None, parent=None):
        ...
        # Splitter horizontal para separar controles e gráfico
        splitter = QSplitter(Qt.Horizontal)
        self.setLayout(QVBoxLayout())
        self.layout().addWidget(splitter)

        # Lado esquerdo: controles e leituras
        left_widget = QWidget()
        ...
        self.voltage_spin.setRange(-505, 505)
        ...
        self.ilim_spin.setRange(2.5e-5, 2.5e-2)
        self.ilim_spin.setDecimals(6)
        self.ilim_spin.setValue(2.5e-5)
        self.ilim_spin.setSingleStep(2.5e-5)
    ...
    def start_measurement(self):
        ...
        if self.user_manager:
            ...
            self.temp_file_path = temp_dir / f"temp_resistance_pico_{timestamp}.txt"
        else:
            self.temp_file_path = None

        # Configuração inicial sequencial com QTimer
    ...
    def configure_instrument_step(self, step):
        """Executa configuração inicial em etapas com QTimer"""
        ...
        try:
            ...
            if step < len(commands):
                ...
                # agenda próxima etapa em 200ms
                QTimer.singleShot(200, lambda: self.configure_instrument_step(step + 1))
            else:
                # Quando terminar a configuração, inicia medições
    ...
    def acquire_reading(self):
        try:
            reading = float(self.instrument.query("READ?"))
            ...
            self.readings.append((relative_time, reading))
        ...
        try:
            ...
            if not os.path.exists(self.temp_file_path):
                with open(self.temp_file_path, 'w') as f:
                    f.write(f"# Auto-save temporário - Medição de Resistência (Pico 6487)\n")
        ...
        if filename:
            ...
            try:
                if filename.endswith('.json'):
                    ...
                    self.ilim_spin.setValue(parameters.get("current_limit", 2.5e-5))
                    ...
                    with open(filename, 'r') as f:
                        for line in f:
                            if line.startswith('#'):
                                if 'Tensão (V):' in line or 'Tensão aplicada (V):' in line:
    ...
    def export_measurement(self):
        ...
        default_filename = f"resistance_pico_{self.start_time.strftime('%Y%m%d_%H%M%S')}.txt"
